...
export const MAX_IMAGE_SIZE_BYTES = 2000000;
...
const validateCloudinaryEnv = ({ uploadUrl, uploadPreset }) => {
  if (isMissingEnvValue(uploadUrl)) {
    return "NEXT_PUBLIC_CLOUDINARY_URL is missing or invalid. Rebuild frontend after setting .env.";
  }

  if (isMissingEnvValue(uploadPreset)) {
    return "NEXT_PUBLIC_CLOUDINARY_UPLOAD_PRESET is missing or invalid. Rebuild frontend after setting .env.";
  }

  const normalizedUrl = normalizeEnvValue(uploadUrl);
  const normalizedPreset = normalizeEnvValue(uploadPreset);

  if (!parseCloudNameFromUploadUrl(normalizedUrl)) {
    return "NEXT_PUBLIC_CLOUDINARY_URL must match https://api.cloudinary.com/v1_1/<cloud_name>/image/upload";
  }

  if (!UPLOAD_PRESET_PATTERN.test(normalizedPreset)) {
    return "NEXT_PUBLIC_CLOUDINARY_UPLOAD_PRESET has an invalid format.";
  }

  return null;
};

export const getFrontendCloudinaryConfig = () => {
  const uploadUrl = process.env.NEXT_PUBLIC_CLOUDINARY_URL;
  const uploadPreset = process.env.NEXT_PUBLIC_CLOUDINARY_UPLOAD_PRESET;
  const validationError = validateCloudinaryEnv({ uploadUrl, uploadPreset });
  ...
  const normalizedUrl = normalizeEnvValue(uploadUrl);
  const cloudName = parseCloudNameFromUploadUrl(normalizedUrl);
