...
import kureHomepageDefaults, { fallbackBrands } from "@utils/kureHomepageDefaults";
import { 
  FiArrowRight, 
  FiChevronRight, 
  FiCheckCircle, 
...
const catBgMap = {
  ...
  "Anti Cancer Drugs":  { light: "#FFF0F0", badge: "#DC2626", accent: "#FEE2E2" },
  ...
  "Lifesaving Medicines": { light: "#FFFBEA", badge: "#D97706", accent: "#FDE68A" },
};

const ProductCard = ({ prod, onEnquire, style = "default" }) => {
  const cat = getTitleString(prod.category?.name || prod.category) || "Medicine";
  const colors = catBgMap[cat] || { light: "#F0F4F8", badge: "#0F4C81", accent: "#DBEAFE" };
  const imageUrl = getProductImageSrc(prod);
  const title = getTitleString(prod.title) || prod.name || "";

  if (style === "deal") {
    return (
      <div className="bg-white rounded-2xl border border-slate-100 hover:border-slate-200/60 shadow-[0_4px_16px_rgba(0,0,0,0.04)] hover:shadow-[0_8px_20px_rgba(15,76,129,0.06)] hover:-translate-y-0.5 transition-all duration-300 flex-shrink-0 group p-3.5 text-center flex flex-col justify-between relative overflow-hidden" style={{ width: 196 }}>
        {/* Category Pill Tag */}
        <span 
          className="absolute top-2.5 left-2.5 text-[10px] font-black px-2 py-0.5 rounded-full border tracking-wider uppercase z-10"
          style={{ background: colors.light, color: colors.badge, borderColor: colors.accent }}
        >
          {cat}
        </span>
        {/* Image area */}
        <div className="relative h-28 w-full flex items-center justify-center bg-white mb-2.5 mt-2 overflow-hidden rounded-lg">
          <img src={imageUrl} alt={title} className="h-full w-full object-contain p-1 transition-transform duration-500 group-hover:scale-105" />
        </div>
        {/* Info */}
        <div className="flex flex-col flex-grow items-center justify-between">
          <div className="w-full">
            <h3 className="text-[13px] font-bold text-slate-800 leading-snug line-clamp-2 min-h-[34px] group-hover:text-[#0F4C81] transition-colors duration-200">{title}</h3>
            <p className="text-[11px] text-slate-400 font-medium truncate mt-1 mb-3">{prod.composition || "Specialty Formulation"}</p>
          </div>
          <div className="mt-auto w-full flex justify-center pb-0.5">
            <Link href={`/product/${prod.slug}`} className="inline-flex items-center justify-center w-full text-[11px] font-extrabold text-[#0F4C81] hover:text-white border border-[#0F4C81]/25 hover:border-[#0F4C81] hover:bg-[#0F4C81] py-1.5 px-3 rounded-lg transition-all duration-200 uppercase tracking-wider">
              Read more
            </Link>
          </div>
        </div>
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-100 hover:border-slate-200/60 shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] hover:shadow-[0_12px_24px_-6px_rgba(15,76,129,0.08)] hover:-translate-y-1 transition-all duration-300 flex flex-col group p-4 text-center relative overflow-hidden">
      {/* Category Pill Tag */}
      ...
      {/* Image */}
      <div className="relative h-36 w-full flex items-center justify-center bg-white mb-3.5 mt-2 overflow-hidden rounded-xl">
        <img src={imageUrl} alt={title} className="h-full w-full object-contain p-2 transition-transform duration-500 group-hover:scale-105" />
      </div>
      {/* Info */}
      <div className="flex flex-col flex-grow items-center justify-between">
        <div className="w-full">
          <h3 className="text-[14px] font-bold text-slate-800 leading-snug line-clamp-2 min-h-[36px] group-hover:text-[#0F4C81] transition-colors duration-200">{title}</h3>
          <p className="text-[12px] text-slate-400 font-medium truncate mt-1 mb-4">{prod.composition || "Specialty Formulation"}</p>
        </div>
        <div className="mt-auto w-full flex justify-center pb-1">
          <Link href={`/product/${prod.slug}`} className="inline-flex items-center justify-center w-full text-[11px] font-extrabold text-[#0F4C81] hover:text-white border border-[#0F4C81]/25 hover:border-[#0F4C81] hover:bg-[#0F4C81] py-2 px-4 rounded-lg transition-all duration-200 uppercase tracking-wider">
            Read more
          </Link>
        </div>
      </div>
...
const Home = ({ featuredProducts, allProducts, homepageSettings, brands }) => {
  ...
  return (
    ...
    >
      ...
      <section className="relative w-full overflow-hidden bg-[#E1E8F0] h-[500px] md:h-[530px] flex items-center select-none group/hero mt-4">
        ...
        <div 
          ...
          style={{ backgroundImage: `url(${slides[currentSlide].bgImage || slides[currentSlide].bg})` }}
        ></div>
        ...
        <div 
          ...
          style={{ 
            clipPath: typeof window !== 'undefined' && window.innerWidth >= 768 ? "polygon(0 0, 88% 0, 100% 100%, 0 100%)" : "none",
        ...
        >
          <div className="max-w-md space-y-4">
            ...
            <h1 className="text-3xl md:text-[38px] font-black leading-[1.12] text-[#0A1D37] tracking-tight transition-all duration-300">
              {slides[currentSlide].titleLine1 || slides[currentSlide].titleText}
              {slides[currentSlide].titleLine1 ? (
                <>
                  <br />
                  <span className="text-[#C1272D]">{slides[currentSlide].titleHighlight}</span>
                  <br />
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pt-4 pb-2 border-t border-slate-100">
              ...
              <div key={idx} className="flex items-center gap-2.5">
                ...
                <div>
                  <div className="text-[10px] font-extrabold text-slate-800 leading-tight">{badge.title}</div>
                  <div className="text-[8px] font-medium text-slate-400 mt-0.5 leading-tight">{badge.description}</div>
                </div>
              </div>
              ))}
            </div>
            ...
            <div className="flex items-center gap-3 pt-2">
              <Link 
                ...
                className="inline-flex items-center gap-1.5 bg-[#0F4C81] text-white text-[11px] font-extrabold px-5 py-3 rounded-lg hover:bg-blue-800 transition-all shadow-sm uppercase tracking-wide"
              >
                {homepageSettings?.hero?.ctaPrimary?.text || "Explore Products"} <span className="text-[9px]">↗</span>
              </Link>
              <button 
                onClick={() => setGenericEnquiryOpen(true)} 
                className="inline-flex items-center gap-1.5 border border-[#0F4C81]/30 text-[#0F4C81] text-[11px] font-extrabold px-5 py-3 rounded-lg hover:bg-slate-50 transition-all cursor-pointer uppercase tracking-wide"
              >
                {homepageSettings?.hero?.ctaSecondary?.text || "Send Enquiry"} <span className="text-[9px]">↗</span>
              </button>
            </div>
          </div>
        </div>
      ...
      {homepageSettings?.qualityBar?.enabled !== false && qualityItems.length > 0 && (
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 lg:gap-8 divide-x divide-white/5">
            {qualityItems.map((item, idx) => (
              <div key={idx} className="flex items-center gap-4 px-4 py-2 lg:first:pl-0">
                ...
                <div>
                  <h4 className="text-white text-[15px] font-black tracking-wide leading-tight">{item.title}</h4>
                  <p className="text-blue-200/70 text-[12.5px] font-semibold mt-0.5 leading-snug">{item.description}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      {homepageSettings?.popularCategories?.enabled !== false && popularCategories.length > 0 && (
      <section className="py-14 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <h2 className="text-3xl font-black text-gray-900 text-center mb-10 tracking-tight">
            {homepageSettings?.popularCategories?.title || "Popular Categories"}
          </h2>
      ...
      {homepageSettings?.bestDeals?.enabled !== false && featuredProducts?.length > 0 && (
        <section className="py-14 bg-[#F8FBFF] border-t border-gray-100">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex items-center justify-between mb-8">
              <h2 className="text-3xl font-black text-gray-900 tracking-tight">
                {homepageSettings?.bestDeals?.title || "Today's Best Deals For You!"}
              </h2>
              <Link href={homepageSettings?.bestDeals?.linkUrl || "/products"} className="text-base font-extrabold text-[#0F4C81] hover:text-[#C1272D] flex items-center gap-1 transition-colors">
                {homepageSettings?.bestDeals?.linkText || "See all"} <FiArrowRight className="w-4 h-4" />
              </Link>
            </div>
            {/* Scrollable row */}
            <div
            ...
            >
              {featuredProducts.map((prod) => (
                <ProductCard key={prod._id} prod={prod} onEnquire={handleEnquireClick} style="deal" />
              ))}
            </div>
          </div>
        </section>
      ...
      {homepageSettings?.promoBanners?.enabled !== false && promoBanners.length > 0 && (
      <section className="py-14 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            {promoBanners.map((banner, idx) => (
            <div key={idx} className="rounded-2xl p-7 flex flex-col justify-between min-h-[160px] relative overflow-hidden" style={{ background: `linear-gradient(135deg, ${banner.gradientFrom} 0%, ${banner.gradientTo} 100%)` }}>
              <div className="absolute top-3 right-3 text-5xl opacity-20">{banner.emoji}</div>
              <div>
                <p className="text-[13px] font-extrabold uppercase tracking-wider mb-2" style={{ color: banner.labelColor }}>{banner.label}</p>
                <h3 className="text-3xl font-black leading-tight" style={{ color: banner.titleColor }}>{banner.title}<br />{banner.titleLine2}</h3>
              </div>
              <Link href={banner.linkUrl} className="inline-flex items-center gap-1 mt-4 text-[14px] font-extrabold hover:underline" style={{ color: banner.linkColor }}>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
            ...
            <div className="lg:col-span-4 space-y-4">
              ...
              <h2 className="text-4xl sm:text-5xl font-black text-gray-900 leading-tight tracking-tight">
                {therapeutics.title} <span className="text-[#0F4C81]">{therapeutics.titleHighlight}</span> {therapeutics.titleSuffix}
              </h2>
            ...
            <div className="lg:col-span-4 space-y-3">
              {(therapeutics.bullets || []).map((item, i) => (
                <div key={i} className="flex items-center justify-between px-4 py-3 bg-white rounded-xl shadow-sm border border-gray-100 hover:border-[#0F4C81]/20 hover:shadow-md transition-all cursor-pointer">
      ...
      {homepageSettings?.featuredBrands?.enabled !== false && featuredBrands.length > 0 && (
      <section className="py-14 bg-white border-t border-gray-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center space-y-8">
          <h2 className="text-2xl font-black text-gray-700 uppercase tracking-widest">
            {homepageSettings?.featuredBrands?.title || "Our Featured Brands"}
          </h2>
          <div className="flex flex-nowrap md:flex-wrap items-center justify-start md:justify-center gap-4 overflow-x-auto pb-4 scrollbar-hide select-none">
            {featuredBrands.map((brand, idx) => (
              ...
              >
                ...
                />
                <span className="hidden text-xs sm:text-sm font-black tracking-tight text-center leading-tight text-gray-500">
                  {brand.name}
                </span>
              </div>
            ))}
      ...
      {homepageSettings?.trendingProducts?.enabled !== false && allProducts?.length > 0 && (
        <section className="py-14 bg-white border-t border-gray-100">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-5 mb-8">
              <div>
                <h2 className="text-3xl font-black text-gray-900 tracking-tight">
                  {homepageSettings?.trendingProducts?.title || "Our Trending Products"}
                </h2>
                <p className="text-base text-gray-400 font-medium mt-1">
                  {homepageSettings?.trendingProducts?.subtitle || "Quality medicines from trusted global partners"}
                </p>
              </div>
              <Link
                href={homepageSettings?.trendingProducts?.linkUrl || "/products"}
              ...
              >
                {homepageSettings?.trendingProducts?.linkText || "See all products"} <FiArrowRight className="w-3.5 h-3.5" />
              </Link>
            </div>

            {/* Category Filter Tabs */}
            <div className="flex items-center gap-2 flex-wrap mb-8">
              {[{ label: "All", key: "" }, ...popularCategories.map((c) => ({ label: c.name, key: c.category }))].map((tab) => (
                <button
                  key={tab.key}
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
              {allProducts
                .filter((p) => !activeCatFilter || getTitleString(p.category?.name || p.category) === activeCatFilter)
                .slice(0, showAllTrending ? undefined : 10)
                .map((prod) => (
                  <ProductCard key={prod._id} prod={prod} onEnquire={handleEnquireClick} style="default" />
                ))}
            </div>

            {/* CTA */}
            {allProducts.filter((p) => !activeCatFilter || getTitleString(p.category?.name || p.category) === activeCatFilter).length > 10 && (
              <div className="text-center mt-10">
                ...
                >
                  {showAllTrending ? "Show Less" : "Show More Products"} <FiArrowRight className={`w-4 h-4 transition-transform duration-200 ${showAllTrending ? "-rotate-90" : ""}`} />
                </button>
              </div>
            )}
          </div>
      ...
      <section className="py-14 bg-[#0F4C81]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col md:flex-row items-center justify-between gap-6">
          <div>
            <h2 className="text-2xl sm:text-3xl font-black text-white">{homepageSettings?.bottomCta?.title}</h2>
            <p className="text-blue-200 text-base font-medium mt-1">{homepageSettings?.bottomCta?.subtitle}</p>
          </div>
          <div className="flex flex-col sm:flex-row gap-3 flex-shrink-0">
            <a href={homepageSettings?.bottomCta?.phoneHref || "tel:+919910768201"} className="inline-flex items-center justify-center gap-2 bg-white text-[#0F4C81] px-7 py-3.5 rounded-lg font-extrabold text-base hover:bg-blue-50 transition-all shadow">
              📞 {homepageSettings?.bottomCta?.phone}
            </a>
            <button onClick={() => setGenericEnquiryOpen(true)} className="inline-flex items-center justify-center gap-2 bg-[#C1272D] text-white px-7 py-3.5 rounded-lg font-extrabold text-base hover:bg-red-700 transition-all shadow cursor-pointer">
              <FiSend className="w-4 h-4 rotate-45" /> {homepageSettings?.bottomCta?.enquiryButtonText || "Send Enquiry"}
            </button>
          </div>
        </div>
      </section>
      ...
      <ProductEnquiryModal modalOpen={genericEnquiryOpen} setModalOpen={setGenericEnquiryOpen} product={generalProductPlaceholder} />
    </Layout>
  );
};
